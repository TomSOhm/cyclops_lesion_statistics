...
def main(smoke: bool = False) -> dict:
    ...
    cov_cols = ["group", "anonyme", "female", "sexe", "pivot_pivot_contact",
                "travail_physique", "tabac", "age_at_trauma", "imc",
                "taille", "poids"]
    ...
    print(f"[1] data: {len(sizes)} patients (composite key OK); "
    ...
    print(f"[1b] date anomalies flagged: {len(anomalies)} -> data_anomalies.csv"
    ...
    age_c = pd.to_numeric(patient.loc[patient.group == "cyclops", "age_at_trauma"],
                          errors="coerce").dropna()
    age_m = pd.to_numeric(patient.loc[patient.group == "meniscus", "age_at_trauma"],
                          errors="coerce").dropna()
    ...
    print(f"[2] Table 1 saved; age SMD = {age_smd:+.3f}; "
    ...
    pf = tf.pf_contrast(wide, value_col="delta_lesion_pf",
                        n_boot=n_boot, n_perm=n_perm, seed=RANDOM_SEED)
    ...
    print(f"[3] PRIMARY PF contrast: Cliff δ = {pf['cliffs_delta']:+.4f} "
          f"({pf['cliffs_delta_magnitude']}), MWU p = {pf['mwu_p']:.5f}, "
          f"perm p = {pf['perm_p']:.5f} ({pf['perm_method']})")
    print(f"    BCa 95% [{pf['bca_lo']:+.3f}, {pf['bca_hi']:+.3f}]; "
          f"inversion 95% [{pf['inv_ci_lo']:+.3f}, {pf['inv_ci_hi']:+.3f}]")

    # --- 4. Sum-6 descriptive (demoted) -----------------------------------
    s6_c = wide.loc[wide.group == "cyclops", "delta_lesion_total"].dropna().astype(float)
    s6_m = wide.loc[wide.group == "meniscus", "delta_lesion_total"].dropna().astype(float)
    from scipy import stats as _st
    sum6 = tf.mwu_with_effects(s6_c.values, s6_m.values, n_boot=n_boot, seed=RANDOM_SEED)
    ...
    print(f"[4] sum-6 (demoted): Cliff δ = {sum6['cliffs_delta']:+.4f}, "
    ...
    for s in SITES:
        ...
        res = tf.mwu_with_effects(cx.values, my.values, n_boot=max(500, n_boot // 5),
                                  seed=RANDOM_SEED)
        wors_c = float((cx > 0).mean())
        wors_m = float((my > 0).mean())
        per_comp_rows.append(dict(
            compartment=s,
    ...
    print(f"[5] per-compartment saved ({len(per_comp)} sites); "
    ...
    for grp in ("cyclops", "meniscus"):
        ...
        m1_results[grp] = dict(k=k, N=N, mean=round(m1["mean"], 4),
                               hdi=[round(m1["hdi_lo"], 4), round(m1["hdi_hi"], 4)])
    results["m1_worsened_pf"] = m1_results
    print(f"[6a] M1 worsened_pf: cyc {m1_results['cyclops']['mean']:.2f} "
    ...
    print("[6b] M3 pooling-structure comparison "
    ...
    if not conv_exch["ok"] and not smoke:
        ...
        esc = bm.fit_m3_with_escalation(df, sites=SITES, pooling="exchangeable",
    ...
    for pool, idt in idatas.items():
        ...
        results["m3_convergence"][pool] = dict(
            ok=bool(c["ok"]), max_rhat=round(float(c["max_rhat"]), 4),
    ...
    v_global = rpt.verdict_bayes_en(idata_exch, "delta_bar",
                                    "H1 global knee-wide (delta-bar)", threshold=0.95,
    ...
    results["m3_derived_pf_contrast_hdi"] = [round(dpf["hdi_lo"], 4), round(dpf["hdi_hi"], 4)]
    ...
    results["m3_2block_contrast_mean"] = round(_post_mean(idata_2b, "contrast_pf_ft"), 4)
    ...
    v_2b = rpt.verdict_bayes_en(idata_2b, "contrast_pf_ft",
    ...
    print(f"     M3 done in {results['m3_fit_seconds']}s; primary(exchangeable) "
    ...
    print(f"     delta-bar global={results['m3_delta_bar_mean']:+.2f} "
          f"HDI{results['m3_delta_bar_hdi']} P(>0)={results['m3_p_delta_bar_gt0']:.3f} "
          f"(honest knee-wide, dilution-prone)")
    print(f"     derived PF-FT (neutral)={results['m3_derived_pf_contrast_mean']:+.2f} "
          f"HDI{results['m3_derived_pf_contrast_hdi']} "
          f"P(>0)={results['m3_derived_pf_contrast_p_gt0']:.3f}")
    print(f"     2-block contrast(candidate)={results['m3_2block_contrast_mean']:+.2f} "
    ...
    isd_mwu = tf.mwu_with_effects(isd_c.values, isd_m.values, n_boot=n_boot,
                                  seed=RANDOM_SEED)
    ...
    aft_cov = [c for c in ("group", "imc", "pivot_pivot_contact")
               if c in merged.columns]
    try:
        m4 = bm.fit_m4_weibull_aft(merged, duration_col="inter_surgery_d",
                                   covariates=aft_cov)
    ...
    t0 = time.time()
    idata_m5 = bm.fit_m5_lognormal(wide, var="inter_surgery_d",
                                   nuts_sampler="nutpie", **mcmc_kwargs)
    conv5 = bm.check_convergence(idata_m5)
    results["m5_convergence"] = dict(
        ok=bool(conv5["ok"]), max_rhat=round(float(conv5["max_rhat"]), 4),
    ...
    results["m4_note"] = ("M4 is a lifelines MLE Weibull AFT (no idata); "
                          "the bayesian H4 artefact is idata_m5.nc.")
    print(f"     isd medians cyc={isd_c.median():.0f} / mén={isd_m.median():.0f} "
          f"(MWU p={isd_mwu['pvalue']:.5f}, δ={isd_mwu['cliffs_delta']:+.2f})")
    print(f"     M5 LogNormal done in {round(time.time()-t0,1)}s; "
    ...
    sens_df = pd.DataFrame([
        dict(model="crude (group only)", odds_ratio=sens["or_crude"],
             pvalue=sens["p_crude"], n=sens["n_crude"]),
        dict(model="adjusted (+ pivot + travail_physique)",
             odds_ratio=sens["or_adjusted"], pvalue=sens["p_adjusted"],
             n=sens["n_adjusted"]),
    ])
    ...
    results["sensitivity_or_adjusted"] = (
        round(float(sens["or_adjusted"]), 4) if np.isfinite(sens["or_adjusted"]) else None
    )
    ...
    print(f"[7] sport/métier sensitivity (worsened_pf): "
    ...
    pf_adj = tf.firth_or(merged, outcome_col="worsened_pf",
                         covariates=("female", "age_at_trauma"))
    results["pf_or_crude"] = round(float(pf_crude["odds_ratio"]), 3)
    results["pf_or_crude_ci"] = [round(pf_crude["or_ci_lo"], 3), round(pf_crude["or_ci_hi"], 3)]
    ...
    results["pf_or_adj_sex_age_ci"] = [round(pf_adj["or_ci_lo"], 3), round(pf_adj["or_ci_hi"], 3)]
    ...
    results["pf_evalue_ci"] = round(float(ev["evalue_ci"]), 3) if ev["evalue_ci"] else None
    # Fuller adjustment incl. derived BMI (sex + age + imc).
    pf_adj_full = tf.firth_or(merged, outcome_col="worsened_pf",
                              covariates=("female", "age_at_trauma", "imc"))
    results["pf_or_adj_sex_age_imc"] = round(float(pf_adj_full["odds_ratio"]), 3)
    results["pf_or_adj_sex_age_imc_ci"] = [round(pf_adj_full["or_ci_lo"], 3),
                                           round(pf_adj_full["or_ci_hi"], 3)]
    results["pf_or_adj_sex_age_imc_n"] = int(pf_adj_full["n"])
    print(f"[7b] PF OR (Firth, {pf_crude['method']}; min cell={pf_crude['min_cell']}): "
    ...
    if anomaly_keys:
        ...
        pf_adj_clean = tf.firth_or(merged[_mask], outcome_col="worsened_pf",
                                   covariates=("female", "age_at_trauma"))
        results["pf_or_adj_sex_age_excl_anomalies"] = round(float(pf_adj_clean["odds_ratio"]), 3)
        results["pf_or_adj_sex_age_excl_n"] = int(pf_adj_clean["n"])
        print(f"     sex+age-adj excl. {len(_akeys)} anomaly patient(s): "
    ...
    pf_delay = tf.firth_or(merged, outcome_col="worsened_pf",
                           covariates=("inter_surgery_d",))
    results["pf_or_adj_delay"] = round(float(pf_delay["odds_ratio"]), 3)
    results["pf_or_adj_delay_ci"] = [round(pf_delay["or_ci_lo"], 3), round(pf_delay["or_ci_hi"], 3)]
    _dly = wide[["inter_surgery_d", "worsened_pf"]].dropna()
    from scipy import stats as _st2
    rho_d, p_d = _st2.spearmanr(_dly["inter_surgery_d"].astype(float),
                                _dly["worsened_pf"].astype(float))
    results["delay_worsened_pf_rho"] = round(float(rho_d), 4)
    results["delay_worsened_pf_rho_p"] = round(float(p_d), 4)
    print(f"     delay-adjusted PF OR {results['pf_or_adj_delay']} "
    ...
    h3 = tf.h3_risk_factors(
        ...
        q=0.10, n_boot=n_boot, seed=RANDOM_SEED,
    )
    ...
    print("=" * 78)
    print(f"M3 primary (exchangeable, δ̄): ok={conv['ok']}  r̂_max={conv['max_rhat']:.4f}  "
    ...
    print(f"M3 structure LOO: best = {results['pooling_loo_best']} "
    ...
    for _pool, _c in results["m3_convergence"].items():
        print(f"  {_pool:14s} ok={_c['ok']} r̂={_c['max_rhat']:.3f} "
              f"ESS={_c['min_ess_bulk']:.0f} div={_c['n_divergent']}")
    print(f"M5 (H4 lognormal): ok={conv5['ok']}  r̂_max={conv5['max_rhat']:.4f}  "
          f"ESS_bulk_min={conv5['min_ess_bulk']:.0f}  divergences={conv5['n_divergent']}")
    print("M4 (Weibull AFT): lifelines MLE — no MCMC convergence "
    ...
    print(rpt.verdict_freq_en(
        pf["mwu_p"], "PF contrast (cyclops vs meniscus, frequentist rank-based)",
        effect=f"Cliff δ = {pf['cliffs_delta']:+.3f} ({pf['cliffs_delta_magnitude']})"))
    print("GLOBAL (honest, dilution-prone): " + v_global["sentence"])
    print(f"DERIVED PF-FT (neutral exchangeable model): mean "
          f"{results['m3_derived_pf_contrast_mean']:+.2f}, "
          f"HDI {results['m3_derived_pf_contrast_hdi']}, "
          f"P(>0)={results['m3_derived_pf_contrast_p_gt0']:.3f}")
    print("CANDIDATE 2-block: " + v_2b["sentence"]
          + f"  | LOO favours: {results['pooling_loo_best']}")
    print(f"PF OR Firth: crude {results['pf_or_crude']} {results['pf_or_crude_ci']}; "
          f"sex+age-adj {results['pf_or_adj_sex_age']} {results['pf_or_adj_sex_age_ci']}; "
          f"E-value {results['pf_evalue_point']}")
    print(f"baseline S1 (6-sum) MWU p = {base['pvalue']:.4f} (ref ≈ 0.78); "
          f"baseline PF TOST p = {results.get('baseline_pf_tost_p')} "
          f"(equivalent={results.get('baseline_pf_equivalent')})")
    print(f"sum-6 δ = {sum6['cliffs_delta']:+.3f}, p = {sum6['pvalue']:.3f} "
          f"two-sided / {p_one:.3f} one-sided  (ref δ≈0.25, p≈0.088/0.044)")
    print(f"inter_surgery_d median cyc={isd_c.median():.0f} / mén={isd_m.median():.0f}  "
          f"(ref 240/518)")
