# Patellofemoral localisation of chondral-lesion progression after cyclops syndrome between two knee surgeries: a conservative knee-wide primary estimand with a non-circular topographic readout  an exploratory paired cyclops–meniscus study
...
flowchart LR
    ...
    E["Meniscus:<br/>two meniscal surgeries"] -.->|"compared with"| D
    classDef grp fill:#e8f0fe,stroke:#3b6db5,color:#11233f;
...
> | Within-cyclops: PF vs FT worsening    |       28 vs 1 |           | Wilcoxon signed-rank p = 2×10⁻⁶                                         |
> | Firth OR for PF worsening          |             |           | 17.2 [2.9, 103.5]                                              |
> | Sex+age-adjusted OR (co-primary)   |             |           | 13.5 [2.3, 80.1]; p = 0.004; E-value 7.77                      |
> | Inter-surgery delay (median, days) |           240 |         528 | p < 0.0001; within-cyclops ρ = −0.03                            |
...
Retrospective, paired cyclops–meniscus study. Each patient contributes two correlated observations (surgeries S1 and S2, ordered by surgery date), so the comparison rests on within-patient change (Δ = S2 − S1) compared between groups. No explicit matching was performed; within-patient correlation is handled by the paired delta and, in the Bayesian model, by a patient random intercept.

- **Cyclops (cyclops):** arthroscopically confirmed cyclops between S1 (ACL reconstruction) and S2 (nodule excision); n = 49 patients (98 rows).
- **Meniscus (meniscus):** two successive meniscal surgeries, no cyclops; n = 20 patients (40 rows).
...
```mermaid
flowchart TB
    subgraph CASES["Cyclops  cyclops · n = 49 (98 rows)"]
        direction LR
        C1["S1<br/>ACL reconstruction"] -->|"cyclops develops"| C2["S2<br/>nodule excision"]
    end
    subgraph CTRL["Meniscus  meniscus · n = 20 (40 rows)"]
        direction LR
...
**Causal structure and the inter-surgery delay.** A directed acyclic graph (DAG) places the inter-surgery delay (`inter_surgery_d`) **downstream** of group (cyclops are re-operated sooner): it is the **observation window / time-at-risk**, not a confounder and **not a biological mediator**  a reading we falsify by showing that PF worsening is unrelated to the delay within cyclops (Spearman ρ ≈ 0). It is **excluded** from the primary progression model (conditioning on a post-group variable would be over-adjustment). Because the window is *shorter* in cyclops, adjusting for it can only *increase* the effect. H4 studies the delay as an *outcome* (a distinct question), using a Weibull AFT (M4, lifelines MLE) and a LogNormal model (M5, Bayesian).

```mermaid
flowchart LR
    G["Group<br/>cyclops vs meniscus"] --> DELAY["Inter-surgery delay<br/>observation window / time-at-risk"]
    G --> PFP["PF chondral progression"]
    DELAY -.->|"not a mediator (within-cyclops ρ ≈ 0)"| PFP
    classDef g fill:#e8f0fe,stroke:#3b6db5,color:#11233f;
    class G g;
```

*Causal DAG. The delay is downstream of group, so it is excluded from the primary model (conditioning on it would be over-adjustment); being shorter in cyclops, it can only inflate  never explain away  the effect.*
...
**Missing data.** After the patient key was fixed to the composite `(group, anonyme)`, **no trauma dates were missing** (the previously reported "seven missing" was an artefact of shared identifiers across sheets) and all 69 patients carry a complete six-compartment outcome at both surgeries (analysable **n = 69**: 49 cyclops + 20 meniscus). The single patient that had formerly been missing all six compartments  a patient operated on the analysis date, clinically a meniscal patient  was completed and **reclassified to the meniscus group** (2026-05-29). Three patients carried anomalous *date values* (two cyclops  one trauma-date drift of 61 days, one negative trauma-to-surgery interval  and one meniscus with a negative interval)  exported for source correction and handled by sensitivity exclusion, not imputation.

## 3. Results

### 3.1 Cohort and baseline balance (Figure 1)

All 69 patients were analysable for progression. Cyclops were somewhat older (median age at trauma 30.9 vs 24.9 years; SMD +0.48) and the sex distribution was imbalanced (sex SMD +0.37; code "2" in 63.3% of cyclops vs 45.0% of meniscus); BMI (derived) was balanced (median 24.4 vs 24.8 kg/m²; SMD −0.08). Crucially, applying a stricter **equivalence (TOST)** test to the S1 cartilage at all three levels (global six-sum, PF block, FT block) under one rule, **only the patellofemoral block  the one that carries the outcome  was positively equivalent** (Mann–Whitney p = 0.818; SMD −0.009; **TOST p = 0.034** against a 0.292 bound). The six-compartment sum and the femorotibial block showed **no detected difference** (Mann–Whitney p = 0.662 and 0.770) yet were **not established as equivalent** by TOST (p = 0.124 and 0.186; SMD +0.19 and +0.26)  the expected power limitation at this sample size, and the reason the causal reading is anchored on the equivalent PF block where the signal later emerged. The imbalanced age and sex covariates were carried into the co-primary adjusted analysis (§3.6).

### 3.2 Primary estimand: knee-wide mean effect (Figure 5)

The pre-registered, partition-invariant primary estimand was **inconclusive**: the knee-wide mean Group × Time effect was **δ̄ = +0.233** (94% HDI [−0.872, +1.453]), with **P(δ̄ > 0 | data) = 0.660**, below the 0.95 directional threshold. This is the **expected dilution** of a mechanically localised signal once it is averaged over six compartments  a deliberately **conservative** test, reported as an honest negative-for-decision result rather than as evidence of no effect or as a failure. The baseline group offset was γ = −3.14 (no direct clinical meaning, since S1 PF scores are equivalent; §3.1). The exchangeable model converged cleanly (max R̂ = 1.003, min ESS_bulk = 1607, 0 divergences; ~73 s).

### 3.3 Patellofemoral localisation and topographic structure (Figures 2, 3, 4, 5, 6, S1)

**Where the signal sits  a non-circular readout.** The **derived patellofemoral contrast**, computed from the same neutral exchangeable posterior (which never saw the partition), was **+2.28** (94% HDI [+0.85, +3.81]), with **P(>0 | data) = 0.997** (two-sided). Descriptively, patellofemoral worsening (Δ_PF > 0) occurred in **57.1% (28/49) of cyclops versus 5.0% (1/20) of meniscus**: a **large** effect (Cliff's δ = +0.535; exact permutation p = 0.0002; Mann–Whitney p = 0.0001), BCa 95% CI [0.367, 0.684], concordant permutation-inversion CI [0.357, 0.675]. The Beta-binomial model estimated a PF worsening probability of 0.569 (94% HDI [0.438, 0.695]) in cyclops versus 0.091 ([0.013, 0.230]) in meniscus  non-overlapping intervals. Within cyclops, the paired comparison was unambiguous: **28 patients worsened in the PF block versus 1 in the FT block** (Wilcoxon signed-rank p = 2×10⁻⁶; rank-biserial = 1.0). Because the FT block is **not** equivalent at baseline (§3.1), this PF-versus-FT localisation is read as **exploratory**: a baseline FT excess in the cyclops group, combined with regression to the mean, could blunt an FT progression signal, so the apparent PF-specificity is hypothesis-generating rather than a clean causal contrast.
...
**Figure 1. Baseline balance.** Standardized mean differences (cyclops − meniscus) for baseline covariates, and the equivalence of S1 cartilage scores (median 0 vs 0; Mann–Whitney p = 0.662). Age (SMD +0.48) and sex (SMD +0.37) are imbalanced; the patellofemoral block is equivalent at S1 (TOST p = 0.034), while the femorotibial block and the 6-sum are not equivalence-established (TOST p = 0.186 and 0.124). (`figures/fig1_baseline_balance.png`)

![Figure 1. Baseline balance.](../figures/fig1_baseline_balance.png)

**Figure 2. Patellofemoral localisation (descriptive).** Distribution of Δ_PF (S2 − S1, trochlea + patella) by group; PF worsening 57.1% (cyclops) vs 5.0% (meniscus); Cliff's δ = +0.535; permutation p = 0.0002. (`figures/fig2_pf_progression.png`)
...
**Figure 4. Within-cyclops localisation.** Paired comparison of PF versus FT worsening within cyclops: 28 patients worsen in PF versus 1 in FT (Wilcoxon signed-rank p = 2×10⁻⁶). (`figures/fig4_topographic_specificity.png`)

![Figure 4. Within-cyclops localisation.](../figures/fig4_topographic_specificity.png)
