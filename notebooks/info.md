...
**Interprétation honnête.** Un intervalle qui contient zéro signale une **absence de preuve** d'un gradient, ce qui n'est pas une **preuve d'absence**. Pour affirmer positivement « pas de gradient », il faudrait que l'intervalle soit entièrement contenu dans une marge d'équivalence posée a priori (par exemple $|\rho| < 0{,}20$, dans la logique TOST côté fréquentiste, ou d'une ROPE côté bayésien). Ce n'est pas le cas : l'intervalle déborde des deux côtés. Le verdict correct est donc **indéterminé** (sous-puissant), ni « effet » ni « équivalence à zéro ». L'argument mécaniste du flexum repose sur la **séparation de groupe** (présent chez $29/49$ cyclops contre $0/20$ méniscus, Fisher $p = 1{,}4\times10^{-6}$), pas sur ce dose-réponse intra-cyclops.

**Limites propres au cas flexum.** Trois facteurs plafonnent la puissance, qu'aucun schéma de ré-échantillonnage ne compense : la mesure du flexum est grossière (degrés entiers, médiane $-3°$, plage $0$ à $-10°$, une vingtaine de cyclops exactement à $0$) ; $\Delta_{\text{PF}}$ est à faible résolution (une vingtaine de zéros sur 49) ; et le flexum est relevé « avant S2 », un instantané qui n'est pas l'exposition intégrée sur l'intervalle S1 à S2. Le bootstrap quantifie correctement l'incertitude, il ne crée pas un signal que la mesure et l'effectif ne contiennent pas.
